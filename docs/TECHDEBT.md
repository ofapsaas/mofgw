...
| 4 | deepseek-v4-flash-0731 sin pricing | ✅ CERRADA 11 Ago (pricing + metadata completa) | BAJA | config prod | **Pricing verificado 08 Ago** (OpenRouter API: $0.09/$0.18/$0.018; directo DeepSeek: cache hit $0.0028, miss $0.14, output $0.28, ctx 1M, max out 384K). **Metadata completa 11 Ago (D4, EPIC-010):** context_window 1,048,576, max_output 384000, thinking [low,high,max], default prescriptivo low, supported_parameters [tools,reasoning], modality text->text — agregada a `model_metadata` en prod + example. Sin pricing propio en la tabla (solo bailian, costo 0). **Tráfico verificado 11 Ago 00:2x (state.json):** flash-0731 SÍ se usa, pero solo del cliente `zot` (291,615 prompt tokens; 231,680 cache-hit + 59,935 miss; output 0; ~0.006% del tráfico flash total). Costo estimado directo DeepSeek ≈ $0.009 acumulado — despreciable, no justifica pricing propio en la tabla. ⚠️ Anuncio oficial de suba "significativa" de precios (Bloomberg 06 Ago) — vigilar. |
...
| 20 | gofmt sucio en `internal/proxy/e2e_009000_test.go` | ✅ HECHA 12 Ago (8a7c96d) | LOW | 009-001 review (preexistente en HEAD) | Deuda preexistente, NO introducida por 009-001 (el diff de la feature excluye el archivo). gofmt reporta diff en HEAD. Correr `gofmt -w` sobre el archivo la próxima vez que se toque (evita ruido de diff en reviews). |
...
| 25 | `thinkingProfile` frágil ante cambios de config (010-002 review Minor #2) | ⏳ ACEPTADA (sin cambio estructural) | LOW | 010-002 review (armed-blue-snipe) | La familia del modelo se deriva iterando `md.Thinking` en `internal/router/router.go` (thinkingProfile): ["always"]→kimi NUNCA, ["adaptive"]→minimax NUNCA, low/medium/max→reasoning, ["high"] exacto→qwen. Orden-dependiente y con shadowing si la config evoluciona. Correcto para los 6 modelos actuales (niveles verificados research §4/§5.4; `config.validate()` protege `thinking_default ∈ thinking`). Si el catálogo evoluciona: agregar campo declarativo `ThinkingFamily` ("reasoning"|"qwen"|"") o unit test que fije la semántica. |
| 26 | qwen3.7-plus max_output corregido 64K→128K (D8) | ✅ HECHA 11 Ago | — | EPIC-010 010-001/010-002 | research §4 decía 64K; verificado 10 Ago en la página del modelo: **max_output 131072** (con y sin thinking); el 64K era la tabla vision-models (max_tokens user-settable). Corregido en `config.go`/`config.yaml` (prod + example) y en el catálogo. Verificado empíricamente en deploy: **bailian acepta max_tokens=131072 para qwen3.7-plus** (200, sin 400). |
| 27 | `bodyHasExplicitEffort` re-parsea el body (010-002 review Nit #5) | ⏳ OPCIONAL | LOW | 010-002 review (armed-blue-snipe) | El router hace `json.Unmarshal` del body solo para chequear la presencia de {reasoning_effort, thinking, enable_thinking}. Optimización: extraer booleans en `ParseChatRequest` (ChatRequest.HasReasoningEffort/etc.). Menor; el body ya está en memoria. |
| 28 | Follower de single-flight reporta `ReasoningTokens`=0 | ✅ HECHA 12 Ago (d573cac) | LOW | fix single-flight 11 Ago (d4727bd) | `singleflight.Usage` (internal/singleflight) replica solo los 4 campos core; el follower de un vuelo deduplicado reconstruye `*provider.Usage` sin `ReasoningTokens` → `recordCacheTokens` reporta reasoning 0 para el follower. No afecta headers `X-Usage-*` ni la facturación por cliente. Si se quiere tracking exacto de reasoning por cliente bajo dedupe: añadir `ReasoningTokens` a `singleflight.Usage` y mapearlo en `fn()` + en la reconstrucción del follower. |
| 29 | Sin timeout de primer token en el chain → upstream colgado quema minutos enteros | ✅ CERRADA 12 Ago (029-001) | MEDIA | Incidente prod 12 Ago (Obs-Radar cron) | El cron Obs-Radar de las 09:00 colgó 902s con 0 tokens: el primer request streamed ~11 min contra go-5 sin emitir primer token (stream interrumpido 09:11:23, chain error 09:15:02 = cancelación del timeout del cron). El chain tiene max_retries 9 + cooldown 60s pero **sin timeout de primer token un intento colgado puede durar indefinidamente** dentro del budget del cliente. **Implementado 029-001 (12 Ago 10:0x):** config `fallback.first_token_timeout` (default 90s, 3 estados, override per-provider) + TTFB timer en el stream que compite contra el primer evento (pcancel + retry mismo provider + rotación circular; después del primer byte gobierna write_timeout). Tests: 4 config + 3 router (-race verde). Deploy: binario 10:06 + restart 10:07, /healthz ok 6/6 providers. Detalle: FINDINGS § 2026-08-12 Obs-Radar. |
| 30 | Chain sin "abstain": agotados retry+switch, devuelve el último error sin semántica de "no hay camino viable" | ✅ **CERRADA 15 Ago 04:2x (ciclo HB)** — brazo abstain implementado (commit 67b37cc, deploy 04:23, healthz 6/6): 5xx/429 agotado → 502 con código semántico `chain_exhausted` (vs 502 plano) + header `Retry-After` (hint del último 429 si existe, si no cooldown del router). 4xx de cliente siguen pasando con su status original = abstain "request insoluble". 8 tests nuevos (4 router + 4 absorb), suite completa -race verde | CERRADA | Scan arXiv 12 Ago (BENCH2ROBUST, https://arxiv.org/abs/2608.11977) | BENCH2ROBUST estudia políticas de tool-use robustas a fallas transitorias/persistentes/silenciosas: retry misma ruta, switch a alternativa, o **abstain** cuando no hay camino viable. Mapeo al chain de mofgw: retry (max_retries 9) + switch (rotación circular) ya existen; falta el tercer brazo — un "abstain" explícito que distinga "falló todo tras N intentos" de "el request es inválido/insoluble" y devuelva al caller un error semántico claro (ej: clase `ErrAllProvidersExhausted` vs 502 plano) + opcionalmente un señal en el response body para que el cliente no reintente en loop. El paper reporta robustness gap casi universal ante inyección de fallas (7 modelos/4 familias) y que combinar conocimiento de recuperación estructurado (BTM, +16.8pp sin retrain) con comportamiento aprendido (RL) llega a 40.8-45.5% bajo inyección — evidencia de que el diseño del recovery context importa. Sin urgencia (failover real raro en prod: failovers=0, cache hit 97.7%), pero queda como candidato de hardening junto a #21/#22. |
...
- 2026-08-15: #30 cerrada (brazo abstain: chain_exhausted + Retry-After, commit 67b37cc, deploy 04:23, healthz 6/6).
- 2026-08-13: #30 agregada (abstain en el chain — candidato del scan arXiv 12 Ago, BENCH2ROBUST https://arxiv.org/abs/2608.11977).
- 2026-08-11: #25-#27 agregadas (EPIC-010: thinkingProfile fragilidad aceptada del review 010-002; qwen max_output 64K→128K cerrada y verificada en deploy; bodyHasExplicitEffort opcional). #4 cerrada completa (metadata flash-0731 en prod) + **tráfico verificado** (solo cliente zot, ~$0.009 acumulado → pricing propio no justificado). #26 verificado empíricamente (bailian acepta max_tokens 131072).
