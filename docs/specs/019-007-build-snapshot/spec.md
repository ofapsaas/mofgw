...
- **P4 — Meta corrupta/ausente → unavailable.** `meta.json` corrupto o ausente, o `api.json` vacío → `Embedded()` retorna `ok=false` → `runSnapshot{Available:false}` → el binario se comporta como hoy (cero fallback, fail-loud sin cache intacto).
